...
#   ser.flush()
  ...
  # cap = cv2.VideoCapture("http://10.42.0.62:8080/?action=stream")
  cap = cv2.VideoCapture("http://192.168.229.144:4747/video", cv2.CAP_FFMPEG)
  ...
  while cap.isOpened():
    ...
    # Count number of "person" detection
    num_persons = sum(1 for detection in detection_result.detections
                  if detection.categories[0].index in labels and labels[detection.categories[0].index] == 'person')

    # for detection in detection_result.detections:
    #   for category in detection.categories:
    #       if category.category_name == 'black-palet':
    #           category.category_name = 'grey-palet'
            
    # Draw keypoints and edges on input image
    image = utils.visualize(image, detection_result)
    image_result = utils.visualize(image, detection_result)
    ...
    persons_text = f'Person: {num_persons}'
